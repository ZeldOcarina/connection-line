...
const blogComments = () => {
	...
	const avatarUrl = document.getElementById('user-data').dataset.userImage;
	const modifyCommentBtns = document.getElementsByClassName('modify-comment-btn');
	...
	if (modifyCommentBtns && modifyCommentBtns.length > 0) {
		for (const btn of modifyCommentBtns)
			btn.addEventListener('click', async (e) => {
				...
				const root = e.target.closest('.comments__comment__name-and-btn-container');
		...
		for (const btn of deleteCommentBtns)
			btn.addEventListener('click', async (e) => {
				try {
					...
					if (response.data === '') displayFlash('success', 'Commento cancellato correttamente');
					setTimeout(() => {
