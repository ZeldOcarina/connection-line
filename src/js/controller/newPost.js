...
import { newPostForm, fileInput, fileInputLabel, editPostImage } from '../model/model';
import { displayFlash } from '../utils/display-alert';
import lastURLWord from '../utils/LastURLWord';

const newPost = () => {
	if (window.location.pathname !== '/blog/post' && lastURLWord() !== 'edit') return;
	...
	newPostForm.addEventListener('input', (e) => {
		...
		if (document.querySelector('.form__input--label')) {
			document.querySelector(
				'.form__input--label'
			).innerHTML = `<img class="form__input--file-icon" src="/assets/icons/upload.png">${e.target.files[0]
				.name}`;
		}
	});
	...
	fileInput.addEventListener('change', (e) => {
		...
		if (files.length > 0 && lastURLWord() !== 'edit') fileInputLabel.textContent = files[0].name;
		if (files.length > 0 && lastURLWord() === 'edit') {
			...
			container.innerHTML = `
					...
					<label class="form__input--label" for="post-image"><img class="form__input--file-icon" src="/assets/icons/upload.png">${files[0]
						.name}</label>
				`;
		}
	});
	...
	newPostForm.addEventListener('submit', async (e) => {
		try {
			...
			form.append('post-image', fileInput.files[0] ? fileInput.files[0] : editPostImage.getAttribute('src'));
