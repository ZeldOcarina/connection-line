...
const storage = multerS3({
  ...
  key: function(req, file, cb) {
    ...
    if (file.fieldname === "post-image")
      cb(
        null,
        sanitize(`${slugify(req.body.title, options)}-${file.originalname}`)
      );
    else if (file.fieldname === "avatar")
      cb(
        null,
        sanitize(
          `${Date.now()}-${slugify(req.body.name, options)}-avatar.${
...
exports.multerErrorChecker = (err, req, res, next) => {
  if (err.name === "MulterError") {
    ...
    return res.status(400).render("error", {
      title: "Upload Error",
      msg:
        "Please try again or send your files manually to info@connectionlinesagl.com"
    });
  }
  next();
};
...
exports.cancellaAvatarFoto = (req, res, next) => {
  if (req.file) {
    ...
    const key = req.user.avatar.split("/")[
      req.user.avatar.split("/").length - 1
    ];
